...
- Run `agentic-setup refresh` for updating AI configurations.
- Run `.claude/hooks/agentic-check-sync.sh` for verifying synchronization.
...
- GitHub actions configurations are located in `.github/` (contains `copilot-instructions.md` and workflows in `.github/workflows/` like `publish-package.yml` and `version-bump.yml`).
...
- Session state validated by `.claude/hooks/agentic-session-freshness.sh`.
